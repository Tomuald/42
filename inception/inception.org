...
#+GITHUB: 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
...
COPY ./conf/www.conf /etc/php/7.3/pool.d/
...
need to use ~wp core / wp cli~. Check out [[https://developer.wordpress.org/cli/commands/core/install/][this tutorial]], this [[https://developer.wordpress.org/cli/commands/core/install/][wp-core]]
docs, this [[https://developer.wordpress.org/cli/commands/user/create/][wp-user]], or this [[https://stackoverflow.com/questions/73744266/install-wordpress-with-cli-in-docker-gives-error-establishing-database-connecti][stackoverflow]] thread.

All right, after some searching, wp core seems to install wp by
itself. This might be way easier than doing the manual install... The
rest might be configured by docker-compose and the like...
...
COPY ./tools/db_install.sh /usr/local/bin/

RUN chmod +x /usr/local/bin/db_install.sh

ENTRYPOINT ["/usr/local/bin/db_install.sh"]
EXPOSE 3306
