...
# this installs php and php7.3-fpm. WHICH I DON'T NEED!
...
RUN apt-get -y update && apt-get -y install wget bash php php-cgi php-mysql php-fpm php-pdo php-gd php-cli php-mbstring
...
COPY tools/db_install.sh /tmp/
RUN chmod +x /tmp/db_install.sh

ENTRYPOINT ["sh", "/tmp/db_install.sh"]
